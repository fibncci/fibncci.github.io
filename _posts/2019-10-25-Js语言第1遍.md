...
```
git status---经常加；
git pull  origin master防止版本号冲突----随时链接已成功的功能；
谁先提交谁不报错；
...
mvc 模型 视图 控制器      多元计算器
不同的层做不同的事情；
基于Flex与PHP的MVC框架整合应用研究 摘要:MVC(模型-视图-控制器)是一种得到了广泛应用的经典复合程序设计模式,使用MVC设计模式能够为软件开发带来低耦合性、高重用性和可适用性、良好的维护性

MVVM（模型-视图-视图模型）是一种基于MVC和MVP的架构模式，它试图更清晰地将用户界面开发从应用程序的业务逻辑与行为中分离。
```
