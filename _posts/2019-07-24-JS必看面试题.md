...
for(i=-(2*n-1);i<=2*n-1;i++){
    for(j=-(2*n-1);j<=2*n-1;j++){
        if(i==0&&j==0){
            // document.writeln("&nbsq");
            document.writeln("&nbsp");
        }
        else if(Math.abs(i)%2!=0 && Math.abs(j)<=Math.abs(i)){
            document.writeln("*");
        }
        else if(Math.abs(j)%2!=0 && Math.abs(i)<=Math.abs(j)){
            document.writeln("*");
        }
        else{
            // document.writeln("$nbsp");
...
for i in range(-b, b+1):
	for j in range(-b,b+1):
		...
		if (i,j) in q  or (i**2+j**2)== 2*a**2 or abs(i)== b or abs(j)==b:
			print("*",end='')
		else:
			print(" ",end="") # 一定要有else
	print()
